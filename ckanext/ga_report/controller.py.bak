...
class GaReport(Blueprint):
    ...
    def index(self):
        ...
        def fill_out_url(url):
            import urlparse
            return urlparse.urljoin(config['ckan.site_url'], url)
...
def _get_top_publishers(limit=20):
    ...
    q = """
        select department_id, sum(pageviews::int) pviews, sum(visits::int) visits
        from ga_url
        where department_id <> ''
          and package_id <> ''
          and url like '/dataset/%%'
          and period_name=%s
        group by department_id order by pviews desc
        """
    if limit:
        q = q + " limit %s;" % (limit)
...
def _get_top_publishers_graph(limit=20):
    ...
    connection = model.Session.connection()
    q = """
        select department_id, sum(pageviews::int) pviews
        from ga_url
        where department_id <> ''
          and package_id <> ''
          and url like '/dataset/%%'
